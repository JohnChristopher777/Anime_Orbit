...
void main() {
  ...
  if (uAutoCenterRepulsion > 0.0) {
    ...
    vec2 repulsion = normalize(uv - centerUV + 1e-4) * (uAutoCenterRepulsion / (centerDist + 0.1));
    uv += repulsion * 0.05;
  } else if (uMouseRepulsion) {
    vec2 mousePosUV = (uMouse * uResolution.xy - focalPx) / max(uResolution.y, 1.0);
    float mouseDist = length(uv - mousePosUV);
    vec2 repulsion = normalize(uv - mousePosUV + 1e-4) * (uRepulsionStrength / (mouseDist + 0.1));
