...
const getWeekNumber = (date: Date) =>
  Math.ceil((Number(date) - Number(new Date(date.getFullYear(), 0, 1))) / 604800000);
...
export default function SeasonPolls() {
  ...
  React.useEffect(() => {
    ...
    getSeasonalAnime("FALL", year, 8)
      .then((items: any[]) => { if (current) setAnime(items || []); })
      .catch(() => { if (current) setAnime([]); })
      .finally(() => { if (current) setLoading(false); });
    return () => { current = false; };
  }, [year]);

  React.useEffect(() => onSnapshot(
    collection(db, "seasonPolls", pollId, "votes"),
    (snapshot) => {
      const next: Record<string, number> = {};
      snapshot.docs.forEach((vote) => { next[vote.id] = Number(vote.data().animeId); });
      setVotes(next);
    },
    () => undefined,
  ), [pollId]);
  ...
  const vote = async (animeId: number) => {
    ...
    try {
      ...
      toast.info("Vote saved on this device. Global sync will retry when available.");
    } finally {
      setVotingId(null);
    }
  };
  ...
  return (
    <section className="season-poll" aria-labelledby="fall-poll-title">
      <header className="season-poll__header">
        <div>
          <span><BarChart3 size={15} /> Vote now · Weekly fan poll</span>
          <h2 id="fall-poll-title">Which Fall anime owns this week?</h2>
          <p>Select one cover to cast your vote. The live results appear after your pick.</p>
        </div>
        <div className="season-poll__meta">
          <span><Users size={14} /> {totalVotes} votes</span>
          <span><Clock3 size={14} /> {closed ? "Voting closed" : "Closes Sunday, 11:59 PM"}</span>
        </div>
      </header>

      <div className="season-poll__options">
        {loading
          ? Array.from({ length: 4 }).map((_, index) => <div key={index} className="season-poll__skeleton" />)
          : anime.length === 0
            ? <div className="season-poll__empty">This week’s Fall lineup is unavailable. Check back shortly.</div>
          : anime.map((item, index) => {
              ...
              return (
                ...
                >
                  {revealResults && <span className="season-poll__fill" style={{ width: `${percentage}%` }} />}
                  <ProgressiveImage
                  ...
                  <span className="season-poll__title">
                    ...
                    <small>{item.type || "TV"}{item.episodes ? ` · ${item.episodes} episodes` : ""}</small>
                  </span>
                  <span className={`season-poll__result ${revealResults ? "is-visible" : ""}`}>
                    {revealResults ? <>{selected && <Check size={13} />}{percentage}%<small>{count} votes</small></> : <small>Choose</small>}
                  </span>
                </button>
              );
            })}
