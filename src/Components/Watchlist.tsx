  ...
  const filteredMangaItems =
    activeMangaFilter === "All"
      ...
      : mangaWatchlist
          ...
          .sort((a, b) => trackerTimestamp(a) - trackerTimestamp(b));
  const sharedItems: (WatchlistItem | MangaWatchlistItem)[] =
    ...
    const itemsHtml = sharedItems
      .map((item) => {
        const title = escapeHtml(
          item.title || item.title_english || "Untitled",
        );
        const status = escapeHtml(
          item.status ||
            (mediaTab === "manga" ? "Plan to Read" : "Plan to Watch"),
        );
        const progressLabel =
          mediaTab === "manga"
...
<body>
  ...
  const handleCopyText = () => {
    const text = sharedItems
      .map(
        (item, i) =>
          `${i + 1}. ${item.title || item.title_english} [${item.status || (mediaTab === "manga" ? "Plan to Read" : "Plan to Watch")}]`,
      )
      .join("\n");
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#18181b] border border-[#ffd700]/40 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
              ...
              <p className="text-xs text-neutral-300 leading-relaxed">
                Export and share your customized anime collection with friends
                or open it as a standalone HTML page.
              </p>
