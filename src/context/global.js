...
//       console.log("Popular Anime Data:", data.data);
...
//       if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
//       const data = await response.json();
//       console.log("Trending Anime Data:", data.data);
...
//         `${baseUrl}/anime?q=${anime}&order_by=popularity&sort=asc`
//       );
//       if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
//       const data = await response.json();
...
export const GlobalContextProvider = ({ children }) => {
  ...
  const getAnimePictures = async (id) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
      const data = await response.json();
      console.log("Anime Pictures Data:", data.data);
      dispatch({ type: GET_PICTURES, payload: data.data || [] });
    } catch (error) {
      console.error("Error fetching anime pictures:", error);
      dispatch({ type: GET_PICTURES, payload: [] });
